...
export const OrderScreens: React.FC<OrderScreensProps> = ({ user, onBack }) => {
      ...
      <ScrollView style={styles.container}>
        <LinearGradient colors={['#3B82F6', '#1E40AF']} style={styles.header}>
          <View style={styles.headerContent}>
            ...
            <Text style={styles.headerTitle}>Order #{selectedOrder.id.slice(-8)}</Text>
            <View style={[styles.statusBadge, { backgroundColor: ORDER_STATUSES[selectedOrder.status as keyof typeof ORDER_STATUSES]?.color }]}>
              <Text style={styles.statusText}>
                {ORDER_STATUSES[selectedOrder.status as keyof typeof ORDER_STATUSES]?.icon} {ORDER_STATUSES[selectedOrder.status as keyof typeof ORDER_STATUSES]?.label}
              </Text>
            </View>
          </View>
        </LinearGradient>
        ...
        <View style={styles.detailsContainer}>
          ...
          <View style={styles.detailsSection}>
            ...
            <View style={styles.summaryRow}>
              ...
              <Text style={styles.summaryValue}>{selectedOrder.totalWeight} kg</Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Total Volume:</Text>
              <Text style={styles.summaryValue}>{selectedOrder.totalVolume} m³</Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Items:</Text>
              <Text style={styles.summaryValue}>{selectedOrder.items.length} items</Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Created:</Text>
              <Text style={styles.summaryValue}>{new Date(selectedOrder.createdAt).toLocaleDateString()}</Text>
            </View>
          </View>
            ...
            {selectedOrder.items.map((item, index) => {
              ...
              return (
                <View key={index} style={styles.detailsItemCard}>
                  <Text style={styles.detailsItemTitle}>{material?.icon} {material?.name}</Text>
                  <Text style={styles.detailsItemDescription}>{item.description}</Text>
                  <Text style={styles.detailsItemDetails}>
                    📦 {item.quantity} {item.unit} • ⚖️ {item.weight}kg
                    {item.volume && ` • 📏 ${item.volume}m³`}
                  </Text>
                </View>
          ...
          {/* Addresses */}
          <View style={styles.detailsSection}>
            <Text style={styles.detailsSectionTitle}>📍 Addresses</Text>
            <View style={styles.addressCard}>
              <Text style={styles.addressTitle}>Pickup Address</Text>
              <Text style={styles.addressText}>
                {selectedOrder.pickupAddress.street}{'\n'}
                {selectedOrder.pickupAddress.city}, {selectedOrder.pickupAddress.state} {selectedOrder.pickupAddress.zipCode}
              </Text>
            </View>
            <View style={styles.addressCard}>
          ...
          >
            <Text style={styles.trackButtonText}>🗺️ Track Order</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    );
  ...
  const renderTrackOrder = () => (
    <ScrollView style={styles.container}>
      <LinearGradient colors={['#8B5CF6', '#7C3AED']} style={styles.header}>
        <View style={styles.headerContent}>
          <TouchableOpacity onPress={() => setCurrentScreen('details')} style={styles.backButton}>
            <Text style={styles.backButtonText}>← Back</Text>
          </TouchableOpacity>
      ...
      <View style={styles.trackingContainer}>
        ...
        <View style={styles.progressContainer}>
          {Object.entries(ORDER_STATUSES).map(([status, info], index) => {
            ...
            const isCompleted = Object.keys(ORDER_STATUSES).indexOf(selectedOrder?.status || '') > index;
            ...
            return (
              <View key={status} style={styles.progressStep}>
                <View style={[
                  ...
                  <Text style={styles.progressIcon}>{info.icon}</Text>
                </View>
                <View style={styles.progressInfo}>
                  ...
                  {isActive && (
                    <Text style={styles.progressTime}>Current Status</Text>
                  )}
                </View>
                {index < 4 && <View style={[styles.progressLine, isCompleted && styles.progressLineCompleted]} />}
              </View>
            );
          })}
        </View>
        ...
        <View style={styles.estimatesContainer}>
          <Text style={styles.estimatesTitle}>📅 Estimated Times</Text>
          {selectedOrder?.scheduledPickupTime && (
            <View style={styles.estimateRow}>
              <Text style={styles.estimateLabel}>Pickup:</Text>
              <Text style={styles.estimateValue}>
                {new Date(selectedOrder.scheduledPickupTime).toLocaleString()}
              </Text>
            </View>
          )}
          {selectedOrder?.scheduledDeliveryTime && (
            <View style={styles.estimateRow}>
              <Text style={styles.estimateLabel}>Delivery:</Text>
              <Text style={styles.estimateValue}>
                {new Date(selectedOrder.scheduledDeliveryTime).toLocaleString()}
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.contactContainer}>
          ...
          <TouchableOpacity style={styles.contactButton}>
            <Text style={styles.contactButtonText}>Chat with Driver</Text>
          </TouchableOpacity>
        </View>
      </View>
    </ScrollView>
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      {currentScreen === 'details' && renderOrderDetails()}
      {currentScreen === 'track' && renderTrackOrder()}
      {renderMaterialModal()}
    </View>
  );
};
