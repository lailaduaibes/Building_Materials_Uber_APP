-- Compatible ASAP Trip Matching Implementation
-- Works with YOUR EXACT existing schema: trip_requests + driver_profiles + driver_locations
...
DO $$
BEGIN
    RAISE NOTICE '✅ ASAP Trip Matching Setup Complete!';
    RAISE NOTICE 'Functions created: find_nearby_available_drivers, start_asap_matching, accept_trip_request, decline_trip_request';
    RAISE NOTICE 'View created: pending_driver_requests';
END $$;

-- Update trip_requests table with proper indexes for ASAP matching
CREATE INDEX IF NOT EXISTS idx_trip_requests_driver_status_deadline 
    ON trip_requests(driver_id, status, acceptance_deadline);

CREATE INDEX IF NOT EXISTS idx_trip_requests_status_deadline 
...
DROP POLICY IF EXISTS "Drivers can update their trip requests" ON trip_requests;  
DROP POLICY IF EXISTS "System can manage trip requests" ON trip_requests;
...
CREATE POLICY "Drivers can update their trip requests"
    ON trip_requests
    FOR UPDATE
    USING (driver_id = auth.uid())
    WITH CHECK (driver_id = auth.uid() AND status IN ('accepted', 'declined'));

CREATE POLICY "System can manage trip requests"
    ON trip_requests
    FOR ALL
    USING (true)
    WITH CHECK (true);

-- Add function to clean expired trip requests
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'trip_requests' AND column_name = 'updated_at'
    ) THEN
...
-- Ensure driver_locations has proper indexes for proximity search
...
-- Function to find nearby drivers (compatible with existing schema)
...
BEGIN
    ...
    WHERE dl.updated_at > NOW() - INTERVAL '1 minute' * min_updated_minutes
    AND (6371 * acos(
        cos(radians(pickup_lat)) * 
        cos(radians(dl.latitude)) * 
        cos(radians(dl.longitude) - radians(pickup_lng)) + 
...
-- Add helpful views for ASAP matching
CREATE OR REPLACE VIEW active_trip_requests AS
SELECT 
    tr.*,
    EXTRACT(EPOCH FROM (acceptance_deadline - NOW()))::INTEGER as seconds_remaining
FROM trip_requests tr
...
-- Show current compatible structure
